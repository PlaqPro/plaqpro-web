...
(function() {
  ...
  const PolygoneMetrage = {
    ...
    _drawCanvas(preview) {
      ...
      for (let m = 0; m <= maxM + 1; m += 0.1) {
        const mR = Math.round(m * 10) / 10;
        const x = toX(mR); const y = toY(mR);
        const isMajor = mR % 5 === 0;
        const isMid1  = !isMajor && mR % 1 === 0;
        const isMid2  = !isMajor && !isMid1 && mR % 0.5 === 0;
        ctx.strokeStyle = isMajor ? 'rgba(148,163,184,.5)' : isMid1 ? 'rgba(148,163,184,.28)' : isMid2 ? 'rgba(148,163,184,.15)' : 'rgba(148,163,184,.07)';
        ctx.lineWidth   = isMajor ? 1 : isMid1 ? 0.5 : 0.3;
